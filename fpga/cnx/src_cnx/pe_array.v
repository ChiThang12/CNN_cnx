...
// Created   : 2026-06-11
// Fix       : 2026-06-11
//   BUG FIX (L3 Dense1 input source):
//     Tr??c: pe_pre_in[j] <= conv2_psum_flat[dense_cnt*256 + j*32 +: 8]
//            conv2_psum_flat là partial sums 32-bit ch?a relu/quant,
//            layout [4][13][8], offset dense_cnt*256 nh?y sai stride.
//     Sau  : pe_pre_in[j] <= conv2_out_flat[dense_cnt*64 + j*8 +: 8]
//            conv2_out_flat là quantized 8-bit output ?úng cho Dense1,
//            m?i group = 8 neurons × 8 bits = 64 bits ? stride = 64.
// Description:
...
//     - L3 (Dense1): 8 activations from conv2_out_flat (quantized Conv2 output)
...
module pe_array(
  ...
  // Conv2 partial sums: 4*13*8*32 = 13312 bits  (dùng cho L2, không dùng cho L3)
  input [13311:0]  conv2_psum_flat,

  // Dense1 partial sums: 6*8*32 = 1536 bits
  input [1535:0]   dense1_psum_flat,

  // Conv2 quantized output: 416*8 = 3328 bits  (FIX: dùng làm input cho L3)
  input [3327:0]   conv2_out_flat,
  ...
  always @(posedge clk or posedge rst) begin
    if(rst) begin
      ...
      // ------------------------------------------------------------------
      // L3 (Dense1): Read 8 activations from conv2_out_flat.
      //   conv2_out_flat: 416 * 8 = 3328 bits (quantized Conv2 output)
      //   Layout: [ch_batch=0..3][col_pos=0..12][PE=0..7], 8-bit each
      //   Packed as: conv2_out_flat[idx*8 +: 8]
      //     where idx = ch_batch*104 + col_pos*8 + PE_j
      //   Group index dense_cnt (0..51) maps directly to a group of 8:
      //     offset = dense_cnt * 64  (8 neurons × 8 bits = 64 bits/group)
      // ------------------------------------------------------------------
      else if(state == L3_RD && counter == 7'd1) begin
        for(j = 0; j < 8; j = j + 1)
          pe_pre_in[j] <= conv2_out_flat[dense_cnt*64 + j*8 +: 8];  // FIX: was conv2_psum_flat[dense_cnt*256 + j*32 +: 8]
        pe_pre_in[8] <= 8'd0;
      end
